...
>
> **En progreso (rama `feature/langgraph-agent`)**: agente nuevo (orchestrator + 4 ramas, `StateGraph` de LangGraph) con completions vía Claude/Foundry y tools vía los MCP `clinyq-mcp-*`, reemplazando `run_conversation_with_rag`/`azure_tools.py` — mismo endpoint, `engine="langgraph"`. Detalle en la sección "LangGraph agent" de `CLAUDE.md`.
