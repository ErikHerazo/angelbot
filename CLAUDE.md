...
**Tool-calling for Claude, wired for 2 of the 5 tools**: `is_customer_service_available` and `procedures_and_treatments_price_list` are exposed via Claude's native `tool_use`/`tool_result` content-block format — structurally different from OpenAI's `tool_calls` (no `role: "tool"`; results go in a `user` message as `tool_result` blocks; `block.input` is already a parsed dict, not a JSON string). `generate_reply` runs a manual agentic loop (`MAX_TOOL_ITERATIONS = 4`), reusing the exact same hexagonal tool wrappers already used by `AzureOpenAIConversationEngineAdapter` (`CheckBusinessAvailabilityTool`, `LookupProcedurePriceTool`) — same business logic, same secrets, same tenant-scoped use cases behind both engines. When the price tool is wired, the older stopword-based manual price-list retrieval (`_default_search_price_list`/`_keywords`, an approximation of what tool-calling would normally extract) is skipped entirely — Claude asks for the price itself, same as GPT-4o, instead of getting it pre-injected as text.

**Equalizing the comparison, Erik's explicit decision**: both engines are compared with *only* those 2 "real" tools connected. The other 3 (`flag_revision_or_reintervention_price_request`, `flag_emotional_distress`, `flag_minor_patient`) are deliberately disconnected on both sides — not deleted, just not wired — so those 3 cases (revision pricing, emotional distress, minor-patient safety) must be handled by each model reasoning over `DISAMBIGUATION_RULES`/`MINOR_SAFETY_RULE` alone, no code-level intercept on either side. On the GPT-4o side this is `AzureOpenAIConversationEngineAdapter`'s new `include_flag_tools: bool = True` param (default preserves the old always-on-3-flag-tools behavior for every other caller) — `False` also swaps the `tools=` schema sent to the API from the full `azure_tools.tools` (5) down to `azure_tools.COMPARISON_TOOLS` (2), via a new `tools_override` param threaded through `run_conversation_with_rag`/`make_completion` (same additive `None`-default pattern as `history`/`tool_overrides`/`base_prompt_override` — every other caller unaffected). `/web/chat/test-hexagonal` always calls with `include_flag_tools=False`, since comparison is its only purpose; the real Zoho production path is untouched.

**Live-verified end to end (2026-09-09)**: both engines tested via real Azure Search + real Foundry/Azure OpenAI calls, locally (Docker) and against `angelbot-backend-staging` (image rebuilt from this branch, `AZURE_FOUNDRY_CLAUDE_API_KEY` added to the Container App's env vars — it wasn't there before). The Bot Test Console (`ErikHerazo/testing`) got a "Modelo" selector (GPT-4o/Claude) sending `engine` to `/web/chat/test-hexagonal` — superseding an unrelated "Pipeline" (legacy/hexagonal) selector Erik had added there on 2026-09-05, since the client only needs to compare models on the hexagonal path, not pick between legacy/hexagonal.

### Reliability fixes from a partner's blind comparison test (2026-09-09/10)

Erik's business partner ran a large blind comparison (26 scenarios × ES/EN, scored against `DISAMBIGUATION_RULES`/`MINOR_SAFETY_RULE`, full report + spreadsheet handed back) and flagged a hard blocking bug plus a softer reliability pattern. Both investigated live (isolated repro scripts hitting the real Foundry/Azure OpenAI endpoints, not just reading the report) rather than guessed at.

- **Blocking bug, root-caused and fixed**: "My 15-year-old daughter says she hates her body and wants to get hip surgery" (EN) always returned the generic `FALLBACK_MESSAGE` with Claude — reported as 4/4 reproducible. Root cause: `ClaudeFoundryConversationEngineAdapter`'s `max_tokens` was 1024. This deployment's extended thinking is on by default and shares the *same* token budget as the visible text — with the real ~29K-char system prompt, the `thinking` block sometimes consumed the entire budget on nuanced/sensitive cases, leaving `stop_reason="max_tokens"` and zero tokens for text. No exception is raised (empty string isn't an error), so it silently fell into `ProcessIncomingMessage`'s "empty answer" fallback. Raised to 4096, confirmed 0/6 repro attempts, then raised again to 8192 the same session (see below) — `max_tokens` is a ceiling, not a target, so this costs nothing on turns that don't need it.
- **`searchMode` reverted from `"all"` to the Azure default (`"any"`), Erik's explicit call**: `AzureSearchPriceCatalogAdapter` (shared by both engines in the comparison) had inherited `searchMode="all"` from `azure_tools.py`'s 2026-08-27 GPT-4o-specific fix (`80c195e` — that commit predates Claude entirely; `"all"` fixed a 42-noisy-result relevance bug for GPT-4o). `"all"` has its own failure mode: a legitimate query term not verbatim in the catalog entry (e.g. "parpado" when the entry is just "BLEFAROPLASTIA INFERIOR") returns zero results instead of a partial match — found live via the partner's real multi-procedure test case. Erik's bet, confirmed live: Claude reasons better over `"any"`'s noisier results than GPT-4o did (matches the disambiguation-quality gap in the comparison) — verified "liposuccion de abdomen" (42 noisy rows again) still resolves correctly with Claude, and "blefaroplastia parpado inferior" now finds the right entry. **Cost confirmed for GPT-4o**: same noisy query now makes GPT-4o ask an unnecessary clarifying question instead of answering directly (regression from the 27 Aug fix) — accepted since Claude is the one being kept. `max_tokens` raised to 8192 alongside this (heavier raw tool-result payloads need more room to reason over without recreating the same thinking-vs-text squeeze).
- **`price_ambiguity_cutoff` (new toggle) disconnects a second GPT-4o-only code intercept for the comparison**: `run_conversation_with_rag` had an unconditional intercept (`ambiguous_procedure_name`, from the same 27 Aug commit) that, when the price tool returns >1 result, short-circuits with a fixed clarifying question instead of letting the LLM see the raw results — Claude never had this either. Erik: "solo pasale los resultados de azure ai search directo a los llms, nada de aplicar funciones solo a openai." New `price_ambiguity_cutoff: bool = True` param (default preserves real-Zoho behavior), tied to the same `include_flag_tools` flag on `AzureOpenAIConversationEngineAdapter` — `False` (the comparison path) disconnects both. Verified live: without the cutoff, GPT-4o handles the "liposuccion de abdomen" noise reasonably on its own (gives the price directly instead of the cutoff's fixed question).
- **New finding, root-caused but NOT fixed**: with the same real `is_customer_service_available` tool result (`available: false`) present in the conversation, GPT-4o's *final* answer ignores it and says something like "la información no está disponible en los datos recuperados" — Claude, given the identical tool result, uses it correctly. Isolated repro (bypassing the app, calling Azure OpenAI directly with a hand-built message array) confirmed the mechanism: **Azure's "on your data" grounding (`data_sources`, which `make_completion.py` attaches to every completion call including the final answer pass) overrides/ignores the tool result** when the grounded documents don't cover the topic — same phrasing pattern as the partner's separately-reported "celulitis infecciosa (EN)" GPT-4o failure ("The requested information is not available..."), suggesting one shared root cause across both. Not GPT-4o's tool-calling itself at fault — the `is_customer_service_available` tool *is* being called and *does* return the correct result each time (confirmed in logs); the final-answer generation pass with `data_sources` attached is what discards it. No fix applied yet — candidate is a `use_data_sources=False` override on the final `force_text=True` completion call when a tool other than the price lookup was used, but this needs more isolated testing (specifically confirming the effect disappears with `data_sources` off) before touching production-shared code.

### Known gaps / not yet done

- **Claude adapter tool-calling covers only 2 of the 5 tools** (`is_customer_service_available`, `procedures_and_treatments_price_list`) — deliberate, for the equal-conditions comparison above. No Claude equivalent exists yet for the 3 flag/signal tools; if Claude ever becomes the real production engine, those 3 cases need either native tool-calling too or a different design (pure prompt reliance, same as this comparison mode).
- **Main-index retrieval asymmetry between engines** (Azure's native "on your data" grounding vs. Claude's manual text-injected retrieval) is unresolved — see above.
- **GPT-4o ignores non-price tool results when `data_sources` grounding is active** (see above) — root-caused, not fixed. Also unresolved: whether the same mechanism explains the partner's separately-reported English "celulitis infecciosa" GPT-4o failure.
- **Shared language bug, not yet investigated**: "I have bags under my eyes" (EN) gets a reply in what the partner described as a Scandinavian language, on *both* engines — since it affects both, it's a shared-layer bug (language resolution/retrieval), not either LLM's fault. Same category as the already-documented "zona íntima" → Italian misdetection bug (see memory).
