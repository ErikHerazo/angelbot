...
Redis-backed conversation history, keyed `session:{session_id}`, TTL 15 min (900s), capped at `MAX_HISTORY=6` messages in `process_zoho_message.py`. Redis connection differs by `APP_ENV` (`prod` uses Azure Redis Enterprise over `rediss://` with a separate password; otherwise local Docker Redis).

**`REDIS_URL_LOCAL` hostname bug, found and fixed 2026-09-05**: `SessionMemoryRedis` defaults to `redis://redis_local:6379` when `REDIS_URL_LOCAL` isn't set, but this repo's `docker-compose.yml` names the service `redis`, not `redis_local` — that default hostname has never actually resolved inside this compose network (verified with `docker exec fast-api getent hosts redis_local` → fails; `getent hosts redis` → resolves). Since Redis reads/writes here are wrapped in try/except that swallow failures, this was likely silently degrading multi-turn memory in local Docker dev without anyone noticing. Fixed by adding `environment: - REDIS_URL_LOCAL=redis://redis:6379` to the `app` and `celery-worker` services in `docker-compose.yml` — this also fixes the hexagonal migration's `RedisConversationHistoryAdapter` (which reads the same env var, defaulting to `127.0.0.1` — correct for bare local/non-Docker runs, also wrong inside a container). If recreating containers to pick this up, use `docker compose` (v2 plugin) rather than the legacy `docker-compose` (v1.29.2) binary — the latter has a known `KeyError: 'ContainerConfig'` bug against images built by newer Docker Engine versions. A parallel `session:{id}:meta` key with independent read/write helpers exists for auxiliary metadata, refreshed alongside the main session TTL. Saving the turn to history is wrapped in try/except in `process_zoho_message.py` — a transient Redis failure logs and is swallowed rather than preventing `send_final_response` from delivering the already-generated answer. The initial history *read* in `run_conversation_with_rag` (`azure_openai.py`) is similarly wrapped in try/except — a transient Redis failure (observed in practice against the Standard-tier Azure Cache used in `prod`, unrelated to any code change — Azure-side metrics showed the cache itself healthy) falls back to an empty history instead of crashing the whole RAG call.
...
Every outbound adapter that's tenant-specific structured/technical config (tenant identity, Zoho screenname/server_uri, business hours, price-catalog index, search indexer name, LLM endpoint/deployments) reads its own named block from **one shared `config/tenants/{tenant_id}/config.yaml`**, via the shared `read_tenant_config_block(config_dir, tenant_id, block_key)` helper (`adapters/outbound/tenant_config/tenant_config_file.py`) — a `tenant:`/`zoho:`/`business_hours:`/`price_catalog:`/`search_indexer:`/`llm:` block each, one file per tenant rather than one file per concern. **Revised 2026-09-16**: this reverses the original per-concern-file convention this migration started with (each of those used to be its own YAML file) — Erik's call, to match the `config.yaml`-with-named-blocks convention already adopted for the standalone `clinyq-mcp-*` repos (see the ClinyQ MCP memory), applied here too for consistency. Plain-text business/legal copy (`greeting.txt`, `continue_message.txt`, `file_upload_ack_message.txt`, `advisor_available_reply.txt`, `agenda_reply.txt`, `flow_confirmation_reply.txt`) deliberately stays as separate `.txt` files, not folded into `config.yaml` — different format for a different reason (non-technical editors, no YAML-escaping hazard), unaffected by this change. Secrets (Zoho access token, Azure AI Search API key, Azure Blob connection string, Azure OpenAI API key) go through `SecretsPort` (`EnvFileSecretsAdapter` today, keyed as `zoho-access-token-{tenant_id}` / `azure-search-api-key-{tenant_id}` / `azure-blob-connection-string-{tenant_id}` / `azure-openai-api-key-{tenant_id}`), never through `config.yaml`.
...
The orchestrator + multi-branch agent design first attempted on `feature/langgraph-migration` (see the earlier "AGB LangGraph migration" work, later lost to a `git reset --hard` and left dangling — never recovered) was **redesigned from scratch** on a new branch, built directly on top of `feature/hexagonal-architecture-migration`'s already-existing ports/use cases rather than the old branch's parallel (and differently-named: `infrastructure/adapters/` vs this migration's `adapters/`) hexagonal layer. Branched off `feature/hexagonal-architecture-migration`, not `feature/switch-to-claude` — this work predates that branch's Claude-comparison code, though it ended up needing a Claude adapter of its own (see below).

### Why a new design instead of recovering the old one

Two things changed since the original design: (1) the tool layer moved out of this repo entirely into 4 standalone MCP servers under `github.com/clinyq` (`clinyq-mcp-azure-search`, `clinyq-mcp-zoho`, `clinyq-mcp-azure-blob`, `clinyq-mcp-azure-translator` — see the ClinyQ MCP memory), so any recovered graph would need its tool-execution nodes rewritten to call MCP anyway; (2) a lot of ports/use cases (`CheckBusinessAvailability`, `ConversationHistoryPort`, `ReplyLanguageResolverPort`, `TranslationPort`, `ReplyCompressionPort`, `PromptConfigRepositoryPort`, etc.) were built on `feature/hexagonal-architecture-migration` after the old branch was reset, so reusing them from scratch is more consistent than reconciling two independently-evolved hexagonal layers. Erik's call, given both options.

### Design decisions made before building

- **Lives inside `AngelBotBackend`**, not a new repo under the `clinyq` org — unlike the MCP servers (which are independently deployable tool services), the orchestrator is core platform logic, tightly coupled to this repo's hexagonal ports/tenant config.
- **`ProcessIncomingMessage` needed zero changes.** It already treats `ConversationEnginePort.generate_reply(...)` as an opaque unit and handles `send_progress_update`/compression/history-save/`send_final_response` generically — so the whole graph only had to implement that one port (`LangGraphConversationEngineAdapter`), exactly like `AzureOpenAIConversationEngineAdapter` does today. Zoho delivery (decision C below) plugs in at a completely different, unrelated seam: which `ChatPlatformPort` implementation the composition root injects into `ProcessIncomingMessage`, not anything the graph touches.
- **Which calls go through MCP vs. stay as direct adapters** (asked and confirmed with Erik before building):
  - Azure Translator (language detection/translation, run 2+ times per turn) — **direct adapter**, reusing the existing `TranslationPort`/`ReplyLanguageResolverPort` Azure adapters as-is. Too high-frequency/low-latency-sensitive to justify a network hop to a separate microservice for no current second consumer.
  - `is_customer_service_available` — **direct, local use case** (`CheckBusinessAvailability`, already built). Pure calc (holidays + tenant config), no external paid API — doesn't fit the "MCP wraps a real external service" criterion the 4 MCP repos were built around. Called deterministically by `orchestrator_node` itself (not LLM-tool-bound), same as the original Confluence-diagram design.
  - Zoho progress/final delivery — **via `clinyq-mcp-zoho`** (`McpZohoChatPlatformAdapter`, implements the existing `ChatPlatformPort`, zero application-layer changes needed).
  - Price catalog + knowledge base search — **via `clinyq-mcp-azure-search`**, LLM-tool-bound (the model decides when to call them, via `RetrievalToolsProviderPort`/`McpRetrievalToolsAdapter`) — replaces both `azure_tools.py`'s price tool and Azure's native "on your data" grounding (Claude has no equivalent to the latter anyway, same retrieval-asymmetry situation already documented for the GPT-4o-vs-Claude comparison).
- **Full 4-branch graph built up front** (orchestrator + retrieval/agenda/direct/flow), not just the retrieval path with stubs — matches the original Confluence diagram and the precedent set building the first version of this graph (Erik asked for the graph shape built before internals back then too). `agenda_agent`/`direct_agent`/`flow_agent` are thin nodes returning per-tenant canned text via 3 new config-backed ports (`AgendaReplyConfigRepositoryPort`, `AdvisorAvailableReplyConfigRepositoryPort`, `FlowConfirmationReplyConfigRepositoryPort`), same one-port-per-canned-message shape as `GreetingConfigRepositoryPort`/`ContinueMessageConfigRepositoryPort`. Canned copy reused verbatim from the equivalent instructions already live in `constants.py`'s prompts (advisor button text, 55€ valoración/agenda link, confirm-contact-details text), not invented.
- **No `mcp`/`fastmcp` client SDK dependency.** Both pull in a dependency chain (`fastmcp` → `starlette>=1.0`; `mcp` → `pyjwt>=2.10.1` → forces `redis-entraid>=1.2` → `azure-identity>=1.24`) that conflicts with this repo's pinned versions — same class of conflict CLAUDE.md already documented once for `fastmcp` alone, now confirmed again from a different transitive path. Fixed by hand-rolling a minimal MCP Streamable-HTTP JSON-RPC client on `httpx` (`McpHttpClient`, `adapters/outbound/mcp/mcp_http_client.py`) instead — validated against a real running `clinyq-mcp-azure-search` container first (confirmed via `curl`: stateless FastMCP needs no `initialize` handshake, a single POST per call is enough; response is always SSE-framed even for a single reply, and the client must send `Accept: application/json, text/event-stream` or get a 406). ~130 lines, fully unit-tested with `httpx.MockTransport`, no new heavyweight dependency.

### The graph (`app/application/use_cases/conversation/`)

Low-level `StateGraph` API (not a prebuilt agent), same architectural stance as the original design: `orchestrator_node` (forced `classify_intent` tool call: `hablar_con_asesor`/`agendar_cita`/`info_general`) routes to `retrieval_agent` (translate query → tool-calling loop against MCP price/KB search, max 4 iterations → final generation), `agenda_agent`/`direct_agent`/`flow_agent` (canned replies), all converging on a shared `enforce_language` node before `END`. `LLMPort` (new port, OpenAI-wire-shaped: plain dicts for messages/tools/tool_calls, `arguments` always a parsed `dict` never a JSON string) is the seam that lets the graph's nodes stay provider-agnostic — two real adapters exist behind it (see next section).

### Completions provider: Claude, not Azure OpenAI (Erik's call, 2026-09-16)

`AzureOpenAILLMAdapter` (direct `AsyncAzureOpenAI` calls, failover logic ported from the legacy `load_balancer.py`) was built first and is a complete, tested `LLMPort` implementation — but Erik decided the actual completions for this agent should run on Claude, so `ClaudeFoundryLLMAdapter` (Sonnet 5 via Microsoft Foundry, same infra pattern as `feature/switch-to-claude`'s `ClaudeFoundryConversationEngineAdapter`) is what `_build_conversation_graph_for_tenant` wires in by default. Unlike that other adapter (which owns the *entire* conversation-engine concern opaquely), this one is *only* the raw completion call — translating between `LLMPort`'s OpenAI-shaped wire format and Claude's native `system`/`tool_use`/`tool_result` format, so no graph node needs to know which provider is answering. The Azure `llm:` config block and `AzureOpenAILLMAdapter` are kept, not deleted (Erik: "dejalo en el config") — the legacy pipeline and the GPT-4o side of the `/web/chat/test-hexagonal` comparison still use Azure OpenAI directly.

**Two real Claude-specific bugs found via a live end-to-end smoke test** (real Foundry + real `clinyq-mcp-azure-search` + real local Redis, `AZURE_OPENAI_API_KEY_AGB`/`AZURE_FOUNDRY_CLAUDE_API_KEY_AGB` added to `.env`), not caught by unit tests with fakes:
1. **"This model does not support assistant message prefill. The conversation must end with a user message."** — `generate_final_node`'s reply-language reinforcement was originally a `role: "system"` message appended mid-conversation (matching the legacy Azure pipeline's approach). Claude only accepts one system prompt (set once, not injected mid-transcript) and requires the transcript to end on a user/tool-result turn before generating — after stripping the trailing system message, the transcript ended on an assistant turn. Fixed by making the reinforcement a `role: "user"` message instead (works for both engines).
2. **The reinforcement message got answered directly instead of steering the real answer** — phrasing it as a bare imperative ("Responde en inglés") made Claude treat it as a brand-new user request ("Understood, I'll respond in English from now on") instead of continuing the substantive answer already in progress. Fixed by explicitly framing it as a non-user system reminder that must continue the prior answer (bracketed, e.g. `"[Recordatorio del sistema, no es un mensaje nuevo del usuario: continúa y completa tu respuesta a la pregunta anterior, en {reply_language}...]"`). Verified with a real English multi-turn call (price question → correct price in English, not an acknowledgement).

Both fixes live in `agents/retrieval_agent.py::generate_final_node` and apply to both engines (no per-provider branching in the node) — `AzureOpenAILLMAdapter` was re-run against the full test suite after each fix and still passes.

### Config: unified `config.yaml` per tenant (Erik's call, 2026-09-16 — reverses the per-concern-file convention above)

While building this, Erik asked to unify `config/tenants/{tenant_id}/`'s several small YAML files into **one `config.yaml` per tenant with a named block per concern** (`tenant:`, `zoho:`, `business_hours:`, `price_catalog:`, `search_indexer:`, `llm:`, `claude:`) — matching the convention already adopted for the `clinyq-mcp-*` repos, applied here too for consistency across the whole ClinyQ codebase family. This **reverses** the "one concern per file" decision documented earlier in this doc for the structured/technical YAML config (tenant identity, Zoho connection, business hours, price catalog, search indexer, LLM config) — all 6 `Filesystem*ConfigRepository` adapters were updated to read their own block from the shared file via a new `read_tenant_config_block(config_dir, tenant_id, block_key)` helper (`adapters/outbound/tenant_config/tenant_config_file.py`), and the old individual files (`tenant.yaml`, `zoho.yaml`, `business_hours.yaml`, `price_catalog.yaml`, `search_indexer.yaml`, `llm.yaml`) were deleted. **Not affected**: the plain-text canned-reply/business-copy files (`greeting.txt`, `continue_message.txt`, `file_upload_ack_message.txt`, `advisor_available_reply.txt`, `agenda_reply.txt`, `flow_confirmation_reply.txt`) and the `prompts/*.yaml` channel-prompt templates — different formats for different reasons (non-technical editors for the `.txt` files; LangChain `load_prompt` compatibility for the prompt YAMLs), untouched by this change.

**Same structure applied one level up, for the agent's own (non-tenant) shared config**: `app/config/agent_config.yaml` — sibling to `app/config/settings.py` and `app/config/tenants/`, same named-block-per-concern shape (`retrieval:`, `mcp_servers:`), for technical config that isn't AGB business data and isn't a fixed Python constant either (Erik drew this distinction explicitly: `config/tenants/agb/config.yaml`'s `claude:` block is legitimately AGB's config even though it feeds the agent, since a future tenant could use a different deployment/quota — but `MAX_TOOL_ITERATIONS` and the `clinyq-mcp-*` server URLs are the *agent's own* config, true regardless of tenant). Named `agent_config.yaml`, not plain `config.yaml` — Erik's call, to avoid the name colliding/confusing with the per-tenant `config.yaml` it sits next to. Only 2 of the 4 `clinyq-mcp-*` servers are listed under `mcp_servers:` (`azure_search_url`, `zoho_url`) — `clinyq-mcp-azure-translator` is deliberately not used by this agent (see the direct-adapter decision above) and `clinyq-mcp-azure-blob` has nothing to do with the conversation graph at all (that's `UploadKnowledgeDocument`/`web/routes/upload_file.py`, a separate administrative flow). `app/config/settings.py` now loads `MAX_TOOL_ITERATIONS`/`MCP_AZURE_SEARCH_URL`/`MCP_ZOHO_URL` from this file at import time (`yaml.safe_load`) instead of hardcoding them as Python literals — every other value in `settings.py` (file-upload limits, language display names, etc.) is untouched, still plain Python constants, since those predate this agent and this change was scoped to the agent's own config only.

### Testing

Unit-tested throughout with hand-written fakes, same convention as the rest of this migration: `McpHttpClient` (`httpx.MockTransport`, plus validated live against a real running `clinyq-mcp-azure-search` container), `McpRetrievalToolsAdapter`/`McpZohoChatPlatformAdapter` (fake MCP client), `AzureOpenAILLMAdapter` (fake `AsyncAzureOpenAI`-shaped client, including a real rate-limit-failover-and-cooldown test), `ClaudeFoundryLLMAdapter` (fake `messages_create_fn`, covers system-message merging, forced tool choice, and the assistant-tool-calls/tool-result round trip), and the graph itself (`tests/application/use_cases/conversation/`, fakes for every port, all 4 routes + the tool-calling loop exercised end-to-end via the real compiled `StateGraph`). 146 tests passing repo-wide after this work.
